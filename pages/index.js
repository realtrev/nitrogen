...
import Auth from '../src/utils/authRoute';
...
export default function Home() {
  ...
  return (
    <Auth forceLogin={false} blockContent={true}>
      <div className="w-screen min-h-screen overflow-x-hidden bg-mid">
        <div className="max-w-4xl mx-auto h-screen flex flex-col">
          <div className="w-full h-20 flex items-center justify-between flex-shrink-0">
            ...
            { session ?
            <button onClick={() => signOut()} className="select-none duration-200 font-bold h-10 px-5 text-red outline-2 focus-visible:outline-primary-1 hover:text-white hover:bg-red rounded-lg bg-white outline-none">
              Log out
            </button>
            :
            <button onClick={() => signIn()} className="select-none duration-200 font-bold h-10 px-5 text-primary-1 outline-2 focus-visible:outline-primary-1 hover:text-white hover:bg-primary-1 rounded-lg bg-primary-2 outline-none">
              Log in
            </button>
            }
          </div>
